...
public class Util {
    ...
    public static List<Post> filterPromos(List<Post> posts) {
        return posts.stream()
                .filter(p -> Boolean.TRUE.equals(p.getHasPromo()) && p.getDiscount() != null && p.getDiscount() > 0)
                .toList();
    }

    public static List<Post> filterByCategory(List<Post> posts, int categoryId) {
        return posts.stream()
                .filter(p -> p.getCategory() == categoryId)
                .toList();
    }

    public static Set<Post> getRecentPosts(Set<Post> posts) {
        return posts.stream()
                .filter(p -> isRecent(p.getDate()))
